...
const getHasRole = async (pager = 1) => {
  pageNo.value = pager
  let res: RoleResponseData = await getRoleList(
      pageNo.value,
      pageSize.value,
      keyword.value,
  )
...
const deleteSelectUser = async () => {
  ...
  let res: any = await RemoveList(idList)
  if (res.code === 200) {
...
const setPermission = async (row: RoleData) => {
  drawer.value = true
  Object.assign(RoleParams, row)
  let res: MenuResponseData = await getMenuList(RoleParams.id as number)
  if (res.code === 200) {
...
const handler = async () => {
  ...
  let res: any = await setMenuList(roleId, permissionId)
  if (res.code === 200) {
    drawer.value = false
    ElMessage({
      type: 'success',
