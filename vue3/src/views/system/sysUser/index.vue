...
const getHasUser = async (pager = 1) => {
  pageNo.value = pager
  let res: UserResponseData = await getPageList(
      pageNo.value,
      pageSize.value,
      keyword.value,
  )
  if (res.code === 200) {
    ...
    userArr.value =res.data.records.map(item=>{
         if(item.status===1){
             item.status=true
         }else{
           item.status=false
...
const switchStatus=  async (row:User)=>{
  row.status = row.status === true? 1 : 0
  let res: any = await reqUpdateStatus(row.id as number,row.status)
  if (res.code) {
    ElMessage({
      type: 'success',
      message: '操作成功',
...
const setRole = async (row: User) => {
  drawer1.value = true
  Object.assign(userParams, row)
  let res: AllRoleResponseData = await getUserRoles(userParams.id as number)
  if (res.code === 200) {
    allRole.value = res.data.allRolesList
    userRole.value = res.data.assginRoleList
    drawer1.value = true
  }
}
...
const deleteUser = async (userId: number) => {
  let res: any = await reqRemove(userId)
  if (res.code === 200) {
